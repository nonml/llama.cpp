...
TURBO_TYPES = ["GGML_TYPE_TBQ4_0", "GGML_TYPE_TBQ3_0", "GGML_TYPE_TBQ2_0"]
...
SOURCE_FATTN_MMA_TURBO_CASE = "DECL_FATTN_MMA_TURBO_CASE({dkq}, {dv}, {ncols1}, {ncols2}, {tK}, {tV});\n"
...
for ncols in [8, 16, 32, 64]:
    for ncols2 in [1, 2, 4, 8]:
        ncols1 = ncols // ncols2
        for tbq_type in TURBO_TYPES:
            for dkq, dv in TURBO_HEAD_SIZES:
                fname = f"fattn-mma-turbo-instance-dkq{dkq}-ncols1_{ncols1}-ncols2_{ncols2}-{get_short_name(tbq_type)}.cu"
                with open(fname, "w") as f:
                    f.write(SOURCE_FATTN_MMA_TURBO_START)
                    f.write(SOURCE_FATTN_MMA_TURBO_CASE.format(
                        dkq=dkq, dv=dv, ncols1=ncols1, ncols2=ncols2,
                        tK=tbq_type, tV=tbq_type))
