...
int get_mmvq_mmid_max_batch(ggml_type type, int cc) {
    // TBQ types do not support MoE ids — return 0 so MUL_MAT_ID skips MMVQ.
    if (type == GGML_TYPE_TBQ4_1S || type == GGML_TYPE_TBQ3_1S) {
        return 0;
    }
...
bool ggml_cuda_should_use_mmvq(enum ggml_type type, int cc, int64_t ne11) {
    if (type == GGML_TYPE_TBQ4_1S || type == GGML_TYPE_TBQ3_1S) {
        // TBQ types use fused WHT kernels — supported for all batch sizes.
...
static void mul_mat_vec_q_switch_type(
    ...
    switch (type_x) {
        ...
        case GGML_TYPE_TBQ4_1S:
            // TBQ4_1S uses a fused WHT kernel; MoE and fusion are not supported.
            GGML_ASSERT(ids    == nullptr && "TBQ4_1S MMVQ does not support MoE ids");
            GGML_ASSERT(fusion.gate == nullptr && fusion.x_bias == nullptr &&
                        "TBQ4_1S MMVQ does not support op fusion");
            mul_mat_vec_tbq4_1s_q8_1_launch(
                vx, vy, dst, ncols_x, nrows_x, ncols_dst,
                stride_row_x, stride_col_y, stride_col_dst,
        ...
        case GGML_TYPE_TBQ3_1S:
            // TBQ3_1S uses a fused WHT kernel; MoE and fusion are not supported.
            GGML_ASSERT(ids    == nullptr && "TBQ3_1S MMVQ does not support MoE ids");
            GGML_ASSERT(fusion.gate == nullptr && fusion.x_bias == nullptr &&
                        "TBQ3_1S MMVQ does not support op fusion");
            mul_mat_vec_tbq3_1s_q8_1_launch(
                vx, vy, dst, ncols_x, nrows_x, ncols_dst,
                stride_row_x, stride_col_y, stride_col_dst,
                nchannels_x, nchannels_y, nchannels_dst,
                stride_channel_x, stride_channel_y, stride_channel_dst,
