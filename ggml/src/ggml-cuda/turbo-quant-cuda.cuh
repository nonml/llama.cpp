...
// === TBQ2: 2-bit codebook (Lloyd-Max for N(0, 1/128)) ===
...
// === TBQ4: 4-bit codebook (Lloyd-Max for N(0, 1/sqrt(128))) ===
...
static __constant__ float d_turbo_wht_signs2[128] = {
    1.0f, 1.0f, 1.0f, 1.0f, -1.0f, 1.0f, 1.0f, -1.0f, 1.0f, -1.0f, -1.0f, -1.0f, 1.0f, -1.0f, -1.0f, -1.0f, 1.0f, 1.0f, -1.0f, -1.0f, 1.0f, -1.0f, 1.0f, -1.0f, 1.0f, -1.0f, -1.0f, 1.0f, -1.0f, 1.0f, 1.0f, 1.0f, 1.0f, 1.0f, -1.0f, -1.0f, -1.0f, 1.0f, -1.0f, -1.0f, -1.0f, -1.0f, -1.0f, -1.0f, 1.0f, 1.0f, 1.0f, -1.0f, 1.0f, -1.0f, 1.0f, 1.0f, 1.0f, -1.0f, -1.0f, 1.0f, -1.0f, -1.0f, -1.0f, -1.0f, -1.0f, -1.0f, 1.0f, 1.0f, 1.0f, -1.0f, 1.0f, -1.0f, -1.0f, -1.0f, -1.0f, 1.0f, -1.0f, 1.0f, -1.0f, 1.0f, -1.0f, -1.0f, 1.0f, 1.0f, -1.0f, 1.0f, -1.0f, 1.0f, 1.0f, -1.0f, 1.0f, -1.0f, -1.0f, -1.0f, -1.0f, 1.0f, -1.0f, -1.0f, 1.0f, -1.0f, 1.0f, -1.0f, 1.0f, 1.0f, 1.0f, -1.0f, -1.0f, 1.0f, -1.0f, 1.0f, -1.0f, 1.0f, 1.0f, -1.0f, -1.0f, 1.0f, -1.0f, 1.0f, -1.0f, 1.0f, 1.0f, -1.0f, 1.0f, -1.0f, 1.0f, -1.0f, -1.0f, -1.0f, -1.0f, -1.0f, 1.0f, -1.0f};
// QJL sign arrays removed — tbq4_0 now uses pure 4-bit PolarQuant (no QJL correction)
...
static __global__ void k_set_rows_tbq3(
    ...
    const int64_t i_base = i * TBQK3_0_GROUP;
    ...
    const int grp_idx = i00 / TBQK3_0_GROUP;
    const int blocks_per_group = TBQK3_0_GROUP / TBQK3_0;
    ...
    for (int b = 0; b < blocks_per_group; b++) {
        block_tbq3_0 & blk = dst_row_ptr[grp_idx * blocks_per_group + b];
        const int off = b * TBQK3_0;
        for (int j = 0; j < TBQK3_0 / 4; j++) blk.qs[j] = 0;
        for (int j = 0; j < TBQK3_0 / 8; j++) blk.signs[j] = 0;
...
// === TBQ3: GET_ROWS dequantize ===
#define QR_TBQ3_0 2
static __device__ __forceinline__
void dequantize_tbq3_0(const void * vx, const int64_t ib, const int iqs, float2 & v) {
    ...
    { const int j = iqs + TBQK3_0 / 2;
      const uint8_t low2 = (x[ib].qs[j/4] >> ((j%4)*2)) & 0x3;
      const uint8_t hi1  = (x[ib].signs[j/8] >> (j%8)) & 0x1;
      v.y = d_turbo_centroids_3bit[low2 | (hi1 << 2)] * norm; }
}

// Temperature scaling for tbq4_0. KLD-optimal: α=1.0 (any scaling hurts KLD).
// Override via TBQ4_ALPHA env var.
static __constant__ float d_tbq4_alpha = 1.0f;

// === TBQ4: SET_ROWS quantize (4-bit PolarQuant, no QJL) ===
static __device__ __forceinline__
void quantize_f32_tbq4_0_block(const float * src, block_tbq4_0 * dst) {
    ...
    dst->norm = __float2half(corrected * d_tbq4_alpha);
}

// === TBQ4: GET_ROWS dequantize ===
#define QR_TBQ4_0 2
static __device__ __forceinline__
...
// === TBQ2: find nearest 2-bit centroid ===
static __device__ __forceinline__
...
static __global__ void k_set_rows_tbq2(
    ...
    const int64_t i = int64_t(blockDim.x) * blockIdx.x + threadIdx.x;
    if (i >= ne_total_groups) return;
    const int64_t i_base = i * TBQK2_0_GROUP;
    uint32_t tmp = (uint32_t)i_base; uint2 div_mod;
    div_mod = fast_div_modulo(tmp, ne00_fd); const int64_t i00 = div_mod.y; tmp = div_mod.x;
    div_mod = fast_div_modulo(tmp, ne01_fd); const int64_t i01 = div_mod.y; tmp = div_mod.x;
    div_mod = fast_div_modulo(tmp, ne02_fd); const int64_t i02 = div_mod.y; const int64_t i03 = div_mod.x;
    ...
    const int64_t dst_row = *(src1 + i01*s10 + i11*s11 + i12*s12);
    const float * grp_src = src0 + i01*s01 + i02*s02 + i03*s03 + i00;
    block_tbq2_0 * dst_row_ptr = (block_tbq2_0 *)((char *)dst + dst_row*s1 + i02*s2 + i03*s3);
    const int grp_idx = i00 / TBQK2_0_GROUP;
    const int blocks_per_group = TBQK2_0_GROUP / TBQK2_0;
    float x[128]; float norm_sq = 0.0f;
    for (int j = 0; j < 128; j++) { x[j] = grp_src[j]; norm_sq += x[j] * x[j]; }
    float grp_norm = sqrtf(norm_sq);
    float inv_norm = grp_norm > 1e-10f ? 1.0f / grp_norm : 0.0f;
    ...
    float recon_norm_sq = 0.0f;
    for (int b = 0; b < blocks_per_group; b++) {
        block_tbq2_0 & blk = dst_row_ptr[grp_idx * blocks_per_group + b];
        const int off = b * TBQK2_0;
        for (int j = 0; j < TBQK2_0 / 4; j++) blk.qs[j] = 0;
...
// === TBQ2: GET_ROWS dequantize ===
#define QR_TBQ2_0 2
static __device__ __forceinline__
void dequantize_tbq2_0(const void * vx, const int64_t ib, const int iqs, float2 & v) {
    const block_tbq2_0 * x = (const block_tbq2_0 *)vx;
    const float norm = __half2float(x[ib].norm);
    { const int j = iqs;
      const uint8_t idx = (x[ib].qs[j/4] >> ((j%4)*2)) & 0x3;
      v.x = d_turbo_centroids_2bit[idx] * norm; }
    { const int j = iqs + TBQK2_0 / 2;
      const uint8_t idx = (x[ib].qs[j/4] >> ((j%4)*2)) & 0x3;
...
static __constant__ float d_tbq3_tcq_codebook[512] = {
...
template<typename idx_t>
static __global__ void __launch_bounds__(512, 1) k_set_rows_tbq3_tcq(
        const float * __restrict__ src0, const idx_t * __restrict__ src1,
        block_tbq3_tcq * __restrict__ dst, const int64_t ne_total_groups,
    ...
    // Compute source and destination pointers (same index math as tbq3_0)
    const int64_t i_base = group * TBQK3_TCQ;
    uint32_t tmp = (uint32_t)i_base; uint2 div_mod;
    div_mod = fast_div_modulo(tmp, ne00_fd); const int64_t i00 = div_mod.y; tmp = div_mod.x;
    div_mod = fast_div_modulo(tmp, ne01_fd); const int64_t i01 = div_mod.y; tmp = div_mod.x;
    div_mod = fast_div_modulo(tmp, ne02_fd); const int64_t i02 = div_mod.y; const int64_t i03 = div_mod.x;
    ...
    const int64_t dst_row = *(src1 + i01*s10 + i11*s11 + i12*s12);
    const float * grp_src = src0 + i01*s01 + i02*s02 + i03*s03 + i00;
    block_tbq3_tcq * dst_blk = (block_tbq3_tcq *)((char *)dst + dst_row*s1 + i02*s2 + i03*s3)
                                  + (i00 / TBQK3_TCQ);
...
#pragma unroll
        ...
        float dist = xt - d_tbq3_tcq_codebook[sid];
        dist = dist * dist;
...
#pragma unroll
        for (int offset = 16; offset > 0; offset >>= 1) {
            float other_cost = __shfl_down_sync(0xFFFFFFFF, best, offset);
    ...
    if (sid < 128) {
        ...
        float c = d_tbq3_tcq_codebook[cur_state];
        my_recon_sq = c * c;
    }
...
#define QR_TBQ3_TCQ 2
static __device__ __forceinline__
void dequantize_tbq3_tcq(const void * vx, const int64_t ib, const int iqs, float2 & v) {
    const block_tbq3_tcq * blk = (const block_tbq3_tcq *)vx + ib;
    ...
    // Decode element iqs
    {
        ...
        v.x = d_tbq3_tcq_codebook[state] * norm;
    }
    // Decode element iqs + 64 (stride = half block size)
    {
        ...
        const uint16_t raw = (uint16_t)blk->qs[byte_idx] | ((uint16_t)blk->qs[byte_idx + 1] << 8);
        const int state = (raw >> bit_off) & 0x1FF;
        v.y = d_tbq3_tcq_codebook[state] * norm;
    }
}

// =====================================================================================
// TBQ2_TCQ: 2-bit Trellis-Coded Quantization (k=2, L=8, 256 states, free initial state)
// =====================================================================================
...
static __constant__ float d_tbq2_tcq_codebook[256] = {
...
// Double-buffered cost arrays + global memory backtrace (128 syncs/group, was 384)
template<typename idx_t>
static __global__ void __launch_bounds__(256, 1) k_set_rows_tbq2_tcq(
        const float * __restrict__ src0, const idx_t * __restrict__ src1,
        block_tbq2_tcq * __restrict__ dst, const int64_t ne_total_groups,
        uint8_t * __restrict__ bt_buf,
        const int use_shared_bt,
        const int64_t ne00, const int64_t ne01, const int64_t ne02,
        const int64_t ne10, const int64_t ne11, const int64_t ne12, const int64_t ne13,
    ...
    const int64_t i_base = int64_t(grp) * TBQK2_TCQ;
    uint32_t tmp = (uint32_t)i_base; uint2 div_mod;
    div_mod = fast_div_modulo(tmp, ne00_fd); const int64_t i00 = div_mod.y; tmp = div_mod.x;
    div_mod = fast_div_modulo(tmp, ne01_fd); const int64_t i01 = div_mod.y; tmp = div_mod.x;
    div_mod = fast_div_modulo(tmp, ne02_fd); const int64_t i02 = div_mod.y; const int64_t i03 = div_mod.x;
    ...
    const int64_t dst_row = *(src1 + i01*s10 + i11*s11 + i12*s12);
    const float * grp_src = src0 + i01*s01 + i02*s02 + i03*s03 + i00;
    block_tbq2_tcq * dst_blk = (block_tbq2_tcq *)((char *)dst + dst_row*s1 + i02*s2 + i03*s3)
                               + (i00 / TBQK2_TCQ);

    // Backtrace: one predecessor byte per 64 low-state groups per step.
    // The predecessor depends only on sid's low 6 bits (same as tbq3_tcq).
    extern __shared__ uint8_t bt_shared[];
    __shared__ float x[128];
    ...
    // use shared memory (same approach as tbq3_tcq).
    if (sid < 128) {
        float v = x[sid] * d_turbo_wht_signs1[sid];
        const int lane = sid & 31;
#pragma unroll
...
#pragma unroll
        ...
        const int pred_idx = sid & 0x3F;
        float dist = xt - d_tbq2_tcq_codebook[sid];
        dist = dist * dist;
...
#pragma unroll
    ...
    if (sid < 128) {
        ...
        float c = d_tbq2_tcq_codebook[cur_state];
        my_recon_sq = c * c;
    }
    cost[sid] = my_recon_sq;
    __syncthreads();
...
// =====================================================================================
// TBQ4_1S: 4-bit WHT-rotated weight quantization (block_size=32, two scales)
// =====================================================================================

// Raw Lloyd-Max centroids for N(0,1) — NOT pre-divided by sqrt(32).
// These match TQ4_0_CENTROIDS in ggml-turbo-quant.c.
static __constant__ float d_tbq4_1s_centroids[16] = {
...
// WHT sign pattern for 32-element blocks — matches TQ3_0_SIGNS in ggml-turbo-quant.c.
static __constant__ float d_tbq4_1s_signs[32] = {
...
// 1/sqrt(32)
#define TBQ4_1S_INV_SQRT32 0.17677669529663688f

// Dequantize one block_tbq4_1s (32 elements) using one warp (32 threads).
// Each thread handles one element, then all threads collaborate on the WHT inverse.
// Launch: grid=(nb, 1, 1), block=(32, 1, 1) — one block per warp.
template<typename dst_t>
static __global__ void dequantize_block_tbq4_1s(const void * __restrict__ vx, dst_t * __restrict__ y, const int64_t k) {
    const int64_t ib   = blockIdx.x;  // one CUDA block = one warp = one block_tbq4_1s
    const int     lane = threadIdx.x; // 0..31

    if (ib * TBQK4_1S >= k) return;

    const block_tbq4_1s * x = (const block_tbq4_1s *)vx + ib;

    // Load 4-bit index and look up centroid * scale
    const uint8_t idx = (x->qs[lane / 2] >> ((lane & 1) * 4)) & 0xF;
    const float d = (lane < 16) ? __half2float(x->d0) : __half2float(x->d1);
    float val = d_tbq4_1s_centroids[idx] * d;

    // WHT inverse: 5 butterfly stages (step = 1, 2, 4, 8, 16)
    // For pair (j, j+step): a = val[j], b = val[j+step], val[j]=a+b, val[j+step]=a-b.
    // With shfl_xor(val, step): thread lane gets val from lane^step.
    // If !(lane & step) → lane is the lower of the pair  → val = val + other
    // If   (lane & step) → lane is the upper of the pair → val = other - val
#pragma unroll
    ...
    // Final multiply by 1/sqrt(32) * sign[lane]
    val *= TBQ4_1S_INV_SQRT32 * d_tbq4_1s_signs[lane];

    y[ib * TBQK4_1S + lane] = ggml_cuda_cast<dst_t>(val);
}

// Launcher: one warp per block (32 elements each)
template<typename dst_t>
static void dequantize_row_tbq4_1s_cuda(const void * vx, dst_t * y, const int64_t k, cudaStream_t stream) {
    GGML_ASSERT(k % TBQK4_1S == 0);
    const int64_t nb = k / TBQK4_1S;
    // grid: (nb, 1, 1), block: (32 threads = 1 warp, 1, 1)
    dequantize_block_tbq4_1s<<<nb, 32, 0, stream>>>(vx, y, k);
}

// =====================================================================================
// TBQ3_1S: 3-bit WHT-rotated weight quantization (block_size=32, two scales)
// =====================================================================================

// Raw Lloyd-Max centroids for N(0,1) — matches TQ3_0_CENTROIDS in ggml-turbo-quant.c.
static __constant__ float d_tbq3_1s_centroids[8] = {
    -1.996684f, -1.291398f, -0.740341f, -0.247508f,
     0.230106f,  0.725222f,  1.277503f,  1.988943f,
};

template<typename dst_t>
static __global__ void dequantize_block_tbq3_1s(const void * __restrict__ vx, dst_t * __restrict__ y, const int64_t k) {
    const int64_t ib   = blockIdx.x;
    const int     lane = threadIdx.x;

    if (ib * TBQK3_1S >= k) return;

    const block_tbq3_1s * x = (const block_tbq3_1s *)vx + ib;

    // Extract 3-bit index: 32 indices packed as 4 groups of 8 in 3 bytes each.
    // Load each group's 3 bytes as a 24-bit integer, shift by (i*3) to get the index.
    const int g = lane / 8;
    const int i = lane % 8;
    ...
    const float d = (lane < 16) ? __half2float(x->d0) : __half2float(x->d1);
    float val = d_tbq3_1s_centroids[idx] * d;

    // WHT inverse (same 32-point transform as TBQ4_1S)
#pragma unroll
    for (int step = 1; step < 32; step <<= 1) {
        const float other = __shfl_xor_sync(0xFFFFFFFF, val, step);
        val = (lane & step) ? (other - val) : (val + other);
    ...
    y[ib * TBQK3_1S + lane] = ggml_cuda_cast<dst_t>(val);
}

template<typename dst_t>
static void dequantize_row_tbq3_1s_cuda(const void * vx, dst_t * y, const int64_t k, cudaStream_t stream) {
    GGML_ASSERT(k % TBQK3_1S == 0);
    const int64_t nb = k / TBQK3_1S;
    dequantize_block_tbq3_1s<<<nb, 32, 0, stream>>>(vx, y, k);
}

// =====================================================================================
// TBQ4_1S / TBQ3_1S: fused mat-vec multiply kernels (WHT-transformed input side)
//
// Math: dot(WHT_inv(centroid*scale), q8) = dot(centroid*scale, WHT_fwd(q8))
// WHT_fwd on q: multiply by signs[lane], apply 5-stage butterfly, scale by 1/sqrt(32)
//
// Kernel layout:
//   grid  = (nrows_x, nchannels_dst, nsamples_dst)
//   block = (32, 1, 1)  — one warp per output element
//   inner loop over ncols_dst tokens (each token uses a different q8_1 slice)
// =====================================================================================

// Fused matmul-vec for TBQ4_1S: one warp per output element, inner loop over tokens.
static __global__ void mul_mat_vec_tbq4_1s_q8_1(
        const void  * __restrict__ vx,            // weight [nrows_x blocks, nblocks_per_row]
        const void  * __restrict__ vy,            // Q8_1 input, multiple tokens
        float       * __restrict__ dst,           // output
        const int   ncols_x,                      // number of weight elements per row
        const int   ncols_dst,                    // number of tokens
        const uint32_t stride_row_x,              // blocks per row in x (= ncols_x / TBQK4_1S)
        const uint32_t stride_col_y,              // Q8_1 blocks per token (= ncols_x_padded / QK8_1)
        const uint32_t stride_col_dst,            // float elements per token in dst
        const uint32_t stride_channel_x,          // x stride in blocks for channel dim
        const uint32_t stride_channel_y,          // y stride in q8_1 blocks for channel dim
        const uint32_t stride_channel_dst,        // dst stride in floats for channel dim
        const uint32_t stride_sample_x,           // x stride in blocks for sample dim
        const uint32_t stride_sample_y,           // y stride in q8_1 blocks for sample dim
        const uint32_t stride_sample_dst) {       // dst stride in floats for sample dim

    const int     row        = blockIdx.x;
    const uint32_t channel_dst = blockIdx.y;
    const uint32_t sample_dst  = blockIdx.z;
    const int     lane       = threadIdx.x;   // 0..31
    const int     nblocks    = ncols_x / TBQK4_1S;

    // x pointer: one channel maps directly (no MoE ids needed here)
    const block_tbq4_1s * x_row =
        (const block_tbq4_1s *)vx
        + sample_dst  * stride_sample_x
        + channel_dst * stride_channel_x
        + row         * stride_row_x;
    ...
    for (int tok = 0; tok < ncols_dst; ++tok) {
        const block_q8_1 * y_tok =
            (const block_q8_1 *)vy
            + sample_dst  * stride_sample_y
            + channel_dst * stride_channel_y
            + tok         * stride_col_y;

        float sum = 0.0f;

        for (int blk = 0; blk < nblocks; ++blk) {
            const block_tbq4_1s * xb = x_row + blk;
            const block_q8_1    * yb = y_tok + blk;   // each TBQ block = one Q8_1 block (both 32 elem)

            // Load Q8_1 value and dequantize: real_val = qs[lane] * d
            float q_val = (float)yb->qs[lane] * __half2float(yb->ds.x);

            // WHT forward on the input side: signs → butterfly → 1/sqrt(32)
            q_val *= d_tbq4_1s_signs[lane];
#pragma unroll
            for (int step = 1; step < 32; step <<= 1) {
                const float other = __shfl_xor_sync(0xFFFFFFFF, q_val, step);
                q_val = (lane & step) ? (other - q_val) : (q_val + other);
            }
            q_val *= TBQ4_1S_INV_SQRT32;

            // Weight: centroid[idx] * scale
            const uint8_t idx = (xb->qs[lane / 2] >> ((lane & 1) * 4)) & 0xF;
            const float   w_scale = (lane < 16) ? __half2float(xb->d0) : __half2float(xb->d1);
            float prod = d_tbq4_1s_centroids[idx] * w_scale * q_val;
...
#pragma unroll
        ...
        if (lane == 0) {
            dst_row[tok * stride_col_dst] = sum;
        }
    }
}

// Fused matmul-vec for TBQ3_1S: one warp per output element, inner loop over tokens.
static __global__ void mul_mat_vec_tbq3_1s_q8_1(
    ...
    const uint32_t sample_dst  = blockIdx.z;
    const int      lane        = threadIdx.x;
    const int      nblocks     = ncols_x / TBQK3_1S;

    const block_tbq3_1s * x_row =
        (const block_tbq3_1s *)vx
        + sample_dst  * stride_sample_x
        + channel_dst * stride_channel_x
        + row         * stride_row_x;
    ...
    for (int tok = 0; tok < ncols_dst; ++tok) {
        ...
        for (int blk = 0; blk < nblocks; ++blk) {
            const block_tbq3_1s * xb = x_row + blk;
            const block_q8_1    * yb = y_tok + blk;
...
#pragma unroll
            ...
            // TBQ3_1S index extraction: 32 indices packed as 4 groups of 8 in 3 bytes each
            const int g = lane / 8;
            const int i = lane % 8;
            ...
            const float w_scale = (lane < 16) ? __half2float(xb->d0) : __half2float(xb->d1);
            float prod = d_tbq3_1s_centroids[idx] * w_scale * q_val;
...
// Host-side launcher for TBQ4_1S MMVQ
static void mul_mat_vec_tbq4_1s_q8_1_launch(
    ...
    GGML_UNUSED(nchannels_x);
    GGML_UNUSED(nchannels_y);
    GGML_UNUSED(nsamples_x);

    const dim3 block_nums(nrows_x, nchannels_dst, nsamples_dst);
    const dim3 block_dims(32, 1, 1);

    mul_mat_vec_tbq4_1s_q8_1<<<block_nums, block_dims, 0, stream>>>(
        vx, vy, dst, ncols_x, ncols_dst,
...
// Host-side launcher for TBQ3_1S MMVQ
static void mul_mat_vec_tbq3_1s_q8_1_launch(
        const void * vx, const void * vy, float * dst,
        const int ncols_x, const int nrows_x, const int ncols_dst,
        const int stride_row_x,   const int stride_col_y,   const int stride_col_dst,
        const int nchannels_x,    const int nchannels_y,    const int nchannels_dst,
    ...
    mul_mat_vec_tbq3_1s_q8_1<<<block_nums, block_dims, 0, stream>>>(
        vx, vy, dst, ncols_x, ncols_dst,
        (uint32_t)stride_row_x,   (uint32_t)stride_col_y,   (uint32_t)stride_col_dst,
        (uint32_t)stride_channel_x, (uint32_t)stride_channel_y, (uint32_t)stride_channel_dst,
        (uint32_t)stride_sample_x, (uint32_t)stride_sample_y, (uint32_t)stride_sample_dst);
...
#define QR_TBQ2_TCQ 2
static __device__ __forceinline__
void dequantize_tbq2_tcq(const void * vx, const int64_t ib, const int iqs, float2 & v) {
    const block_tbq2_tcq * blk = (const block_tbq2_tcq *)vx + ib;
    const float norm = __half2float(blk->norm);
    ...
    // Decode element iqs: read 8-bit state via sliding window
    {
        ...
        v.x = d_tbq2_tcq_codebook[state] * norm;
    }
    // Decode element iqs + 64
    {
        ...
        const uint16_t raw = (uint16_t)blk->qs[byte_idx] | ((uint16_t)blk->qs[byte_idx + 1] << 8);
        const int state = (raw >> bit_off) & 0xFF;
        v.y = d_tbq2_tcq_codebook[state] * norm;
    }
}
